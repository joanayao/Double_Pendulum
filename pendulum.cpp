...
DoublePendulum::DoublePendulum(double lenght1_, double lenght2_, double theta1_, double theta2_, double gravity_){
    ...
    function<double(Point)> acel_2 = [this](Point P){
        return (G*sin(P.theta2)-lenght_2*P.vel2*P.vel2*sin(P.theta1)*cos(P.theta1)*(cos(P.theta2)*cos(P.theta2)-sin(P.theta2)*sin(P.theta2))
            -G*sin(P.theta1)*(cos(P.theta1)*cos(P.theta2)+sin(P.theta1)*sin(P.theta2))
            -lenght_1*P.vel1*P.vel1*(cos(P.theta1)*sin(P.theta2)-sin(P.theta1)*cos(P.theta2)))
    ...
    function<double(Point)> acel_1 = [this](Point P){
        return -(G*sin(P.theta2)-lenght_2*P.vel2*P.vel2*sin(P.theta1)*cos(P.theta1)*(cos(P.theta2)*cos(P.theta2)-sin(P.theta2)*sin(P.theta2))
            ...
            -lenght_2*P.vel2*P.vel2*(sin(P.theta1)*cos(P.theta2)-cos(P.theta1)*sin(P.theta2))-G*sin(P.theta1);
    };
